...
class Async_Bracha_Functionality(UCWrappedFunctionality):
    ...
    def party_input(self, pid, inp):
        if pid == 1:
            for p in self.parties:
                ...
                self.write('f2w', ('schedule', self.send_output, ((self.sid,p), inp)), 0)
...
from async_ours import Async_Channel, Async_Bracha_Protocol, Async_FWrapper
from adversary import DummyWrappedAdversary

def asyncBrachaSimulator(prot):
    def f(sid, pid, channels, pump, poly, importargs):
        return Async_RBC_Simulator(sid, pid, channels, pump, prot, poly, importargs)
    return f

class Async_RBC_Simulator(ITM):
    def __init__(self, sid, pid, channels, pump, prot, poly, importargs):
        ...
        self.sim_channels,static,_pump = createWrappedUC([('F_chan', Async_Channel)], wrappedProtocolWrapper(prot), Async_FWrapper, DummyWrappedAdversary, poly, importargs={'ctx': self.ctx, 'impflag':False})
    ...
    def add_output_schedule(self, leak, pid_idx):
        self.tick(1)
        _,idx,fname = leak
        self.internal_run_queue.insert(idx,leak)
        self.pid_to_queue[pid_idx] = idx
        self.internal_delay += 1
