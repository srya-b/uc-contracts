...
class Syn_Bracha_Protocol(UCWrappedProtocol):
    ...
    def clock_round(self):
        #self.p2w.write( ('clock-round',) )
        self.write('p2w', ('clock-round',))
    ...
    def send_msg(self, to, msg):
        ...
        self.write('p2f', ((fchannelsid,'F_chan'), ('send',msg)))
        m = wait_for(self.f2p); assert m.msg[1] == 'OK', str(m)

    def val_msg(self, sender, inp):
        ...
        if not self.prepared_value and sender[1] == 1:
            ...
            for pid in self.except_me():
                self.send_msg( pid, ('ECHO', self.prepared_value) )
            self.num_echos[inp] += 1
        dump.dump()
   
    def echo_msg(self, inp):
        n = len(self.parties)
        self.num_echos[inp] += 1
