...
THe logic behind the wrapper is to enable synchronous communication in UC.
...
synchronous execution of code blocks. The protocol parties and functionalities
...
-- ``leak''       : there are two was of leaking information to the adversary:
                    1). directly write the leak onto the tape of the adversary (activating it)
...
class Syn_FWrapper(UCWrapper):
    def __init__(self, k, bits, channels, pump, poly, importargs):
        self.curr_round = 1
        self.delay = 0
        ...
        UCWrapper.__init__(self, k, bits, 'wrap', 'me', channels, poly, pump, importargs)
